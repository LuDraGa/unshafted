...
export const SiteStrip = () => {
    ...
    return (
      <section className="mb-4 flex items-center gap-3 rounded-2xl border border-stone-200 bg-stone-50 px-3 py-2">
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-semibold text-[var(--unshafted-text-soft)]">{state.hostname}</p>
          <p className="text-xs text-[var(--unshafted-text-faint)]">Not analysed — you can still read its policies</p>
        </div>
        <button
          className="flex-shrink-0 rounded-xl border border-stone-300 bg-white px-3 py-1.5 text-xs font-semibold text-[var(--unshafted-text-soft)] transition hover:bg-stone-100"
          onClick={openPanel}
          type="button"
          disabled={tabId === null}>
          {/*
  ...
  return (
    <section className={`mb-4 flex items-center gap-3 rounded-2xl border px-3 py-2 ${RISK_TONE[riskLevel]}`}>
      ...
      <button
        className="flex-shrink-0 rounded-xl bg-white/70 px-3 py-1.5 text-xs font-bold text-[var(--unshafted-text)] transition hover:bg-white"
        onClick={openPanel}
        type="button"
        disabled={tabId === null}>
        What you agreed to
